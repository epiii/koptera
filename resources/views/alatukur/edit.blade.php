      ...
      <div class="row">
        <div class="col-md-8 col-md-offset-2">
          <div class="panel panel-default">{{-- panel --}}
              <div class="panel-heading">Ubah UTTP </div>

              <div class="panel-body">{{-- -body--}}
                <form action="{{ route('alatukur.update',$alatukur) }}" method="post">
                  {{ csrf_field() }}
                  {{ method_field('PATCH')}}

                  {{-- {{ Form::hidden('invisible', 'secret') }} --}}
                  {{-- <input type="hidden" name="id_user" value="{{$id_jenisuttp}}"> --}}
                  ...
                  <div class="form-group">
                    ...
                    <select required class="form-control" name="id_uttp" id="id_uttp">
                      ...
                      @foreach ($uttps as $uttp)
                        <option {{$alatukur->id_uttp==$uttp->id?'selected':''}} {{ old('id_uttp')==$uttp->id?'selected':'' }} value="{{$uttp->id}}">{{$uttp->nama}}</option>
                      @endforeach
                    </select>
                  </div>
...
{{-- {{dd(is_nan($id_jenisuttp))}} --}}
                  ...
                  <div class="form-group nonspbu has-feedback{{ $errors->has('kapasitas')?'has-error':''}}">
